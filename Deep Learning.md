...
不需要把输入图像分割成子集，分别执行前向传播，而是把它们作为一张图片给卷积网络进行计算，其中的公共区域可以共享很多计算。
...
YOLO 算法：只看一次。将对象分配到其中点所在的格子，即使对象可以横跨多个格子。卷积实现。



### 交并比

交并比函数（IoU），用来评价对象检测算法，计算两个边界框交集和并集之比。

如果 >= 0.5，检测正确，是阈值。越高，边界框越精确。



### 非极大值抑制

只输出概率最大的分类结果，抑制很接近但不是最大的其他预测结果。
...
分配和对象形状交并比最高的 anchor box。

问题：如果有两个 anchor box，但在同一个格子中有三个对象；两个对象都分配到一个格子中，并且它们的anchor box 形状相同。

怎样选择 anchor box？一般手工指定形状。k - 平均算法：将两类对象形状聚类，选择最具有代表性的一组 anchor box，是自动选择 anchor box 的高级方法。



### 候选区域

R - CNN 算法：带区域的卷积网络，尝试选出一些区域，在这些区域上运行卷积网络分类器是有意义的，不再针对每个滑动窗运行检测算法。

选出候选区域的方法是运行图像分割算法。

快速的 R - CNN 算法：用卷积实现了滑动窗法。最初的算法是逐一对区域分类的。
...
人脸验证 v.s. 人脸识别：人脸验证，1 对 1 问题，是否和他声称的身份相符；人脸识别，1 对多问题。



### One - Shot 学习

一次学习问题：需要通过单单一张图片或单单一个人脸样例就能去识别这个人。

Similarity 函数：以两张图片作为输入，输出两张图片的差异值。



### Siamese 网络

将输入图片的距离定义为这两幅图片编码之差的范数。

Siamese 网络架构：对于两个不同的输入，运行相同的卷积神经网络，然后比较它们。



### Triplet 损失

通过学习神经网络参数，来得到优质人脸图片编码的方法之一：定义三元组损失函数，然后应用梯度下降。

三元组损失：通常会同时看三张图片，anchor 图片、positive 图片和 negative 图片。如果所有图像的 f 都是一个零向量，总能满足方程。为了确保网络对于所有的编码不会总是输出  0，也不会把所有的编码都设成相等的，需要修改目标，不能是刚好小于等于 0，应该是比 0 还小，称为间隔。
